...
function filterPirates (pirates) {
  ...
  for(i = 0; i < pirates.length; i++) {  
    ...
    var tahunLahir = 2020 - pirates[i].age;
    var strTahunLahir = tahunLahir.toString();
    ...
    var lokasi = '';
      ...
      if(lokasi == 'JKT' || lokasi == 'BGR' || lokasi == 'DPK' || lokasi == 'BKS'){
                  objPirates = {
                    name: pirates[i].name,
                    ID: `${strTahunLahir[2]}${strTahunLahir[3]}-${lokasi}`
                    }

                    let penyusup = false;
                    for(j = 0; j < pirates[i].name.length; j++){
